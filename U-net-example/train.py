# 전처리작업을 끝낸 데이터셋 npy파일을 불러와서 학습을 진행하는 파일입니다.
...
img_rows = 256 #이미지 크기가 256*256이어야함
img_cols = 256

smooth = 1.
# 이부분은 학습을 진행하면서 보게될 loss값과 다이스코피값을 계산하는 함수입니다.
...
#앞서 설명드린 유넷모델의 원리 그림에서 인풋을 받아들이고 아웃풋을 출력하는 부분입니다.
def get_unet1():
...
def train_and_predict():
    ...
    print('Fitting model...')
    print('-'*30)
    model.fit(imgs_train, imgs_mask_train, batch_size=20, epochs=200, verbose=1, shuffle=True,
